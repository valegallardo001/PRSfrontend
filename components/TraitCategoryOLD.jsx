    ...
    if (selectedAncestries.length === 0) {
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/trait-categories`)
        ...
        .then((data) => {
          ...
          console.log("Datos recibidos del backend por ancestry:", data);
          const mergedByName = {};

          colored.forEach((cat) => {
            const name = cat.name;
            if (!mergedByName[name]) {
              mergedByName[name] = {
                ...cat,
                traits: [...(cat.traits || [])],
                pgss: cat.pgss || 0,
              ...
              mergedByName[name].pgss += cat.pgss || 0;
              mergedByName[name].traits.push(...(cat.traits || []));
            }
          });

          setCategories(Object.values(mergedByName));
...
*/
  ...
  const chartData = useMemo(() => {
    ...
    categories.forEach((cat) => {
      const name = cat.label || cat.name; // usa label si existe
      const value = cat.pgss || 0;
      ...
      if (!grouped[name]) {
        ...
        grouped[name].value += value; // evita duplicar categorías
      }
    });
